...
let currentPage = 1;
let productsPerPage = productList
	? Number(productList.getAttribute("data-item"))
	: 12;
let productsData = [];
...
function fetchProducts() {
	fetch(`${BASE_URL}Test/products`)
		...
		.then((data) => {
			...
			const layoutItems = productContainer.querySelectorAll(
				".choose-layout .item"
			);
			...
			function handleFiltersChange() {
				selectedFilters = {
					type: document
						.querySelector(".filter-type .active")
						?.getAttribute("data-item"),
					subcategory: document
						.querySelector(".filter-subcategory .active")
						?.getAttribute("data-item"),
					subcategoryType: document
						.querySelector(".filter-subcategory-type .active")
						?.getAttribute("data-item"),
					size: Array.from(
						document.querySelectorAll(".filter-size .size-item.active")
					).map((item) => item.getAttribute("data-item")),
					color: Array.from(
						document.querySelectorAll(".filter-color .color-item.active")
					).map((item) => item.getAttribute("data-item")),
					brand: Array.from(
					...
					minPrice: 0, //default
					maxPrice: 50000, //default
					sale: document.querySelector(
						'.check-sale input[type="checkbox"]:checked'
					),
				};
				...
				if (rangeInput && rangeInput.length > 1) {
					...
					if (document.querySelector(".filter-price select")) {
						const selectPrice = document.querySelector(
							".filter-price select"
						).value;
						if (selectPrice !== "null") {
							const [min, max] = selectPrice
								.split("-")
							...
							selectedFilters.maxPrice = 50000;
						}
					}
				}
				...
				let filteredProducts = productsData.filter((product) => {
					...
					if (
						...
						(!Array.isArray(product.subcategory_slug) || !product.subcategory_slug.includes(selectedFilters.subcategory))
					)
						return false;
					if (
						selectedFilters.subcategoryType &&
						selectedFilters.subcategoryType?.length > 0 &&
						(!Array.isArray(product.subcategory_type_slug) || !product.subcategory_type_slug.includes(selectedFilters.subcategoryType))
					)
						return false;
					if (
					...
					if (
						...
						!product.variation.some((variant) =>
				...
				let newHtmlListFiltered = `
                    ...
                    <div class="list flex items-center gap-3">
						...
						? `
                                <div class="item flex items-center px-2 py-1 gap-1 bg-linear rounded-full capitalize" data-type="subcategory">
                                    <i class='ph ph-x cursor-pointer'></i>
                                    <span>${selectedFilters.subcategory}</span>
                                </div>
                            `
						: ""
					}
                        ${selectedFilters.subcategoryType?.length
						? `
                                <div class="item flex items-center px-2 py-1 gap-1 bg-linear rounded-full capitalize" data-type="subcategory-type">
                                    <i class='ph ph-x cursor-pointer'></i>
                                    <span>${selectedFilters.subcategoryType}</span>
                                </div>
                            `
						: ""
					}
                        ...
                        ${typeof selectedFilters.brand === "object" &&
				...
				const clearBtnItem = document.querySelectorAll(
					".list-filtered .list .item"
				);
				...
				clearBtnItem.forEach((btn) => {
					btn.addEventListener("click", () => {
						...
						document
							.querySelectorAll(`.filter-${dataType} .active`)
							?.forEach((item) => item.classList.remove("active"));
						if (document.querySelector(`.filter-${dataType} select`))
							document.querySelector(`.filter-${dataType} select`).value = null;
				...
				clearBtn?.addEventListener("click", () => {
					document
						.querySelectorAll(".filter-type .active")
						?.forEach((item) => item.classList.remove("active"));
					document
						.querySelectorAll(".filter-subcategory .active")
						?.forEach((item) => item.classList.remove("active"));
					document
						.querySelectorAll(".filter-subcategory-type .active")
						?.forEach((item) => item.classList.remove("active"));
					document
						.querySelectorAll(".filter-size .active")
						?.forEach((item) => item.classList.remove("active"));
					document
						.querySelectorAll(".filter-color .active")
						?.forEach((item) => item.classList.remove("active"));
					document
						.querySelectorAll(
							'.filter-brand .brand-item input[type="checkbox"]:checked'
					...
					if (
						document.querySelector('.check-sale input[type="checkbox"]:checked')
					) {
			...
			const checkboxBrandItems = document.querySelectorAll(
				'.filter-brand .brand-item input[type="checkbox"]'
			);
			...
			// Get filter type from url. Prefer window.initialProductFilters
			// (set server-side from the validated /products/{category}/{subcategory}/{subcategory_type}
			// path segments); fall back to the query string for any stray
			// un-redirected legacy link.
			const pathname = new URL(window.location.href);
			const initialFilters = window.initialProductFilters || {};
			const typeUrlParam = pathname.searchParams.get("type");
			const categoryUrlParam = initialFilters.category || pathname.searchParams.get("category");
			const subcategoryUrlParam = initialFilters.subcategory || pathname.searchParams.get("subcategory");
			const subcategoryTypeUrlParam = initialFilters.subcategory_type || pathname.searchParams.get("subcategory_type");

			// Hierarchical filter initialization
			if (!typeUrlParam && !categoryUrlParam && !subcategoryUrlParam && !subcategoryTypeUrlParam) {
				// Default on /products with no query params: Clear all
				localStorage.removeItem("selectedType");
				typeItems.forEach(item => item.classList.remove("active"));
				document.querySelectorAll(".filter-subcategory .item").forEach(item => item.classList.remove("active"));
				document.querySelectorAll(".filter-subcategory-type .item").forEach(item => item.classList.remove("active"));
			} else {
				let activeType = typeUrlParam || categoryUrlParam;
				let activeSubcategory = subcategoryUrlParam;
				let activeSubcategoryType = subcategoryTypeUrlParam;

				// If subcategory type is provided, find its parents from productsData
				if (activeSubcategoryType) {
					const match = productsData.find(p => Array.isArray(p.subcategory_type_slug) && p.subcategory_type_slug.includes(activeSubcategoryType));
					if (match) {
						activeType = activeType || match.type;
						activeSubcategory = activeSubcategory || (Array.isArray(match.subcategory_slug) ? match.subcategory_slug[0] : match.subcategory_slug);
					}
				} else if (activeSubcategory) {
					const match = productsData.find(p => Array.isArray(p.subcategory_slug) && p.subcategory_slug.includes(activeSubcategory));
					if (match) {
						activeType = activeType || match.type;
					}
				}
				...
				if (activeType) {
					localStorage.setItem("selectedType", activeType);
					typeItems.forEach(item => {
						if (item.getAttribute("data-item") === activeType) item.classList.add("active");
						else item.classList.remove("active");
					});
				}

				if (activeSubcategory) {
					document.querySelectorAll(".filter-subcategory .item").forEach(item => {
						if (item.getAttribute("data-item") === activeSubcategory) item.classList.add("active");
						else item.classList.remove("active");
					});
				}

				if (activeSubcategoryType) {
					document.querySelectorAll(".filter-subcategory-type .item").forEach(item => {
						if (item.getAttribute("data-item") === activeSubcategoryType) item.classList.add("active");
						else item.classList.remove("active");
					});
				}
			}
			...
			typeItems.forEach((item) => {
				item.addEventListener("click", () => {
					...
					typeItems.forEach((it) => {
						if (it.getAttribute("data-item") === item.getAttribute("data-item")) it.classList.add("active");
						else it.classList.remove("active");
					});

					// When a category is clicked, clear its children filters
					document.querySelectorAll(".filter-subcategory .item").forEach(it => it.classList.remove("active"));
					document.querySelectorAll(".filter-subcategory-type .item").forEach(it => it.classList.remove("active"));

					handleFiltersChange();
				});

				if (item.querySelector(".number")) {
					item.querySelector(".number").innerHTML = productsData.filter(
						(product) => product.type === item.getAttribute("data-item")
					).length;
				}
			});

			const subcategoryItems = document.querySelectorAll(".filter-subcategory .item");
			subcategoryItems.forEach((item) => {
				item.addEventListener("click", () => {
					subcategoryItems.forEach((it) => it.classList.remove("active"));
					item.classList.add("active");

					// Automatically activate parent category
					const match = productsData.find(p => Array.isArray(p.subcategory_slug) && p.subcategory_slug.includes(item.getAttribute("data-item")));
					if (match) {
						typeItems.forEach(it => {
							if (it.getAttribute("data-item") === match.type) it.classList.add("active");
							else it.classList.remove("active");
						});
						localStorage.setItem("selectedType", match.type);
					}

					// Clear subcategory types when a new subcategory is selected
					document.querySelectorAll(".filter-subcategory-type .item").forEach(it => it.classList.remove("active"));
				...
				if (item.querySelector(".number")) {
					item.querySelector(".number").innerHTML = productsData.filter(
						(product) => Array.isArray(product.subcategory_slug) && product.subcategory_slug.includes(item.getAttribute("data-item"))
					).length;
				}
			});

			const subcategoryTypeItems = document.querySelectorAll(".filter-subcategory-type .item");
			subcategoryTypeItems.forEach((item) => {
				item.addEventListener("click", () => {
					subcategoryTypeItems.forEach((it) => it.classList.remove("active"));
					item.classList.add("active");

					// Automatically activate parent subcategory and category
					const match = productsData.find(p => Array.isArray(p.subcategory_type_slug) && p.subcategory_type_slug.includes(item.getAttribute("data-item")));
					if (match) {
						// Activate Subcategory
						document.querySelectorAll(".filter-subcategory .item").forEach(it => {
							if (Array.isArray(match.subcategory_slug) && match.subcategory_slug.includes(it.getAttribute("data-item"))) it.classList.add("active");
							else it.classList.remove("active");
						});
				...
				if (item.querySelector(".number")) {
					item.querySelector(".number").innerHTML = productsData.filter(
						(product) => Array.isArray(product.subcategory_type_slug) && product.subcategory_type_slug.includes(item.getAttribute("data-item"))
					).length;
				}
			});
			...
			if (document.querySelector(".filter-type select")) {
				document
					.querySelector(".filter-type select")
			...
			if (document.querySelector(".filter-size select")) {
				document
					.querySelector(".filter-size select")
					.addEventListener("change", handleFiltersChange);
			}
			...
			if (document.querySelector(".filter-color select")) {
				document
					.querySelector(".filter-color select")
					.addEventListener("change", handleFiltersChange);
			}
			...
			brandItems.forEach((item) => {
				if (item.querySelector(".number")) {
					item.querySelector(".number").innerHTML = productsData.filter(
						(product) => product.brand === item.getAttribute("data-item")
					).length;
				}
			});
			...
			if (document.querySelector(".filter-brand select")) {
				document
					.querySelector(".filter-brand select")
					.addEventListener("change", handleFiltersChange);
			}
			...
			if (document.querySelector(".filter-price select")) {
				document
					.querySelector(".filter-price select")
					.addEventListener("change", handleFiltersChange);
			}
...
function renderProducts(page, products = []) {
	...
	displayedProducts.forEach((product) => {
		...
		let productTags = "";
		if (product.new) {
			productTags += `<div class="sagar product-tag text-button-uppercase bg-green px-3 py-0.5 inline-block rounded-full absolute top-3 left-3 z-[1]">New</div>`;
		}
		if (product.sale) {
			productTags += `<div class="sagar product-tag text-button-uppercase text-white bg-red px-3 py-0.5 inline-block rounded-full absolute top-3 left-3 z-[1]">Sale</div>`;
		}
		if (product.outOfStock) {
			productTags += `<div class="sagar product-tag text-button-uppercase text-white bg-black px-3 py-0.5 inline-block rounded-full absolute top-3 left-3 z-[1]">Out of Stock</div>`;
		}
		...
		const disableActionsIfOutOfStock = () => {
		...
		if (productContainer.classList.contains("style-grid")) {
			...
			productItem.innerHTML = `
					...
					: `
                                        <div
                                            class="hidden quick-shop-btn text-button-uppercase py-2 text-center rounded-full duration-500 bg-white hover:bg-black hover:text-white max-lg:hidden">
                                            Quick Shop</div>
                                        <div
					...
					100
                                        ...
                                        <div class="flex items-center justify-between gap-3 gap-y-1 flex-wrap mt-2">
                                            <div class="text-button-uppercase">
                                                ...
                                                <span class='max-sm:text-xs'>${product.sold
				}</span>
                                            </div>
                                            <div class="text-button-uppercase">
                                                <span class='text-secondary2 max-sm:text-xs'>Available:
                                                </span>
                                                <span class='max-sm:text-xs'>${product.quantity -
				product.sold
				}</span>
                                            </div>
                                        </div>
						...
						?.map(
							(item, index) =>
								`<div
                                                    ...
                                                    >
                                                        <div class="tag-action bg-black text-white caption2 capitalize px-1.5 py-0.5 rounded-sm">${item.colorName || item.color}</div>
                                                    </div>
                                                    `
						)
						.join("")}
			...
			productItem.innerHTML = `
                ...
                `
					...
					: ""
                                    ...
                                    <div class='text-secondary desc mt-5 max-sm:hidden'>${product.description
				}</div>
                                </div>
                                <div class="action w-fit flex flex-col items-center justify-center">
                                    ...
                                    <div class="list-action-right flex items-center justify-center gap-3 mt-4">
                                        ...
                                        <div
                                            class="similar-product-btn w-[32px] h-[32px] flex items-center justify-center rounded-full bg-white duration-300 relative"
                                            data-category="${product.category}"
                                        >
                                            <div class="tag-action bg-black text-white caption2 px-1.5 py-0.5 rounded-sm" style="width: max-content;">
                                                Similar Products</div>
                                        ...
                                        <div
                                            class="hidden compare-btn w-[32px] h-[32px] flex items-center justify-center rounded-full bg-white duration-300 relative">
                                            <div class="tag-action bg-black text-white caption2 px-1.5 py-0.5 rounded-sm">
                                                Compare Product</div>
                                            <i class="ph ph-arrow-counter-clockwise text-lg compare-icon"></i>
                                            <i class="ph ph-check-circle text-lg checked-icon"></i>
